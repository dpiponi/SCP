...
<body>
  ...
  <script>
    ...
    const hwDigitSegEls = [];
    const hwDotEls = [];
    const hwKeyElsById = Object.create(null);
    ...
    function renderHardware() {{
      ...
      for (let i = 0; i < 9; i++) {{
        ...
        const digitEl = hwDigitEls[i];
        if (!digitEl) continue;
        digitEl.classList.toggle("active", active === i);
        digitEl.classList.toggle("selected", selected === i);
        digitEl.title = `digit ${i} seg=${hex(seg, 2)} reg=${state.dsReg[i] & 1}`;
        const segEls = hwDigitSegEls[i];
        for (let s = 0; s < HW_SEG_NAMES.length; s++) {{
          segEls[s].classList.toggle("on", lit.has(HW_SEG_NAMES[s]));
        }}
        hwDotEls[i].classList.toggle("on", !!(seg & 0x80));
      }}
      const scanRow = decodedScanRow(state.D);
      for (const key of KEYS) {{
        const btn = hwKeyElsById[key.id];
        if (!btn) continue;
        btn.classList.toggle("active", pressedKeys.has(key.id));
        btn.classList.toggle("scanned", key.row === scanRow);
      }}
    }}
